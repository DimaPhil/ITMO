...
struct HW2 {
    ...
    void add_reflexive_proof(std::vector<std::string> &result, const std::string &alpha) {
    ...
    void run(const char *input, const char *output) {
        ...
        std::vector<std::string> hypothesis_strings = Utils::split(split_line[0], ',');
        std::string alpha = hypothesis_strings.back();
        Expression *alpha_expression = parse_expression(alpha);
        if (check_class_inherity<BinaryOperation>(*alpha_expression)) {
            alpha = "(" + alpha + ")";
        }
        hypothesis_strings.pop_back();

        std::set<size_t> hypothesis;
        for (size_t i = 0; i < hypothesis_strings.size(); i++) {
            hypothesis.insert(parse_expression(hypothesis_strings[i])->hash());
        }
        ...
        for (size_t i = 0; i < hypothesis_strings.size(); i++) {
            std::cout << hypothesis_strings[i];
            if (i + 1 < hypothesis_strings.size()) {
                std::cout << ",";
            }
        }
        std::cout << "|-";
