...
void TcpServerSocket::acceptConnection(const epoll_event &event) {
    ...
    pendingConstructorHandler = [=]() {
        ...
        r = getnameinfo(&in_addr, in_len, hostbuf, sizeof hostbuf, portbuf, sizeof portbuf, NI_NUMERICHOST | NI_NUMERICSERV);
        if (r != 0) {
        ...
        std::unique_ptr<TcpSocket> socket(new TcpSocket(executor, incomingfd, hostbuf, portbuf));
