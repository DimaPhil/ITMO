...
bool isNewLine(const string &data, int index) {
...
bool HttpObject::append(const string &data) {
    ...
    if (mHasBody) {
        ...
        while (getline(stream, line)) {
            ...
            string key = toLower(trim(line.substr(0, position)));
...
void HttpObject::addHeader(const string &key, const string &value) {
    headers.push_back(std::make_pair(toLower(key), toLower(value)));
}
...
void HttpObject::setHeader(const string &key, const string &value) {
    ...
    headers.push_back(std::make_pair(lowerKey, toLower(value)));
}

void HttpObject::addHeaders(const std::vector<std::pair<string, string>> &headers) {
    for (size_t i = 0; i != headers.size(); ++i) {
        this->headers.push_back(std::make_pair(toLower(headers[i].first), headers[i].second));
    }
}
